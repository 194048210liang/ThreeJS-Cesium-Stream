...
function render() {
  ...
  chart.setOption({
    ...
    xAxis: { type: 'category', data: data.map((d: any) => d.name), axisLine: { lineStyle: { color: 'rgba(0,160,255,0.3)' } }, axisLabel: { color: '#7a8a9a' } },
    ...
    series: [{ type: 'bar', data: data.map((d: any) => d.value), barWidth: 28, itemStyle: { color: new echarts.graphic.LinearGradient(0, 0, 0, 1, [{ offset: 0, color: '#00a0ff' }, { offset: 1, color: '#0050b3' }]), borderRadius: [4, 4, 0, 0] } }],
  })
}
